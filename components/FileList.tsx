...
export default function FileList({
    ...
    const handleDownloadFile = async (file: FileType) => {
        try {
            ...
            // For images, we can use the ImageKit URL directly with optimised settings
    ...
    const openImageViewer = (file: FileType) => {
        if (file.type.startsWith("image/")) {
            ...
            // Using higher quality and responsive sizzing for better viewing experience 
    ...
    const navigateToFolder = (folderId: string, folderName: string) => {
        ...
        // notify parent componet about folder change 
        if (onFolderChange) {
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="overflow-x-auto">
                        ...
                        >
                            <TableHeader>
                                ...
                                <TableColumn width={246}>Actions</TableColumn>
                            </TableHeader>
                            <TableBody>
                                {filteredFiles.map((file) => (
                                    <TableRow
                                        key={file.id}
                                        className={`hover:bg-default-100 transition-colors ${file.isFolder ||
                                            file.type.startsWith("image/") ? "cursor-pointer" : ""
                                        }`}
            ...
            <ConfirmationModal 
                ...
                isDangerous={true}
                warningMessage={`You are about to permanently delete all ${trashCount} items in your trash. These files will be permanently removed fromm your account and cannot be recovered.`}
            />
        </div>
    )
}